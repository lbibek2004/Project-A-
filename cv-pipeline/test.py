...
# import cv2
# img = cv2.imread("test_images/1000004739.jpg")
...
# # Load the image
# img = cv2.imread('test_images/1000004741.jpg')
...
def scan_document(image_path):
    ...
    # 3. Find Contours
    # Look for the shapes in the edged image and sort them by size
    contours, _ = cv2.findContours(edged.copy(), cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    contours = sorted(contours, key=cv2.contourArea, reverse=True)[:5]

    document_contour = None

    # Loop over the largest contours to find the document
    for c in contours:
        # Approximate the contour to a polygon
        peri = cv2.arcLength(c, True)
        approx = cv2.approxPolyDP(c, 0.02 * peri, True)

        # If our polygon has exactly 4 points, we've likely found the paper
        if len(approx) == 4:
    ...
    cv2.imshow("Scanned Document", scanned_effect)
...
# Run the scanner
scan_document("test_images/good_img.jpg")